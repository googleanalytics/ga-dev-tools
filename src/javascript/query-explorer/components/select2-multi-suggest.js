 ...
 * @return {string} The rendered HTML string.
 */
function s2DropdownItemTemplate(opt) {
  let data = opt.element ? $(opt.element).data() : opt;
  return `
    <div class="S2Tag">
      <span class="S2Tag-category">${data.group}</span>
      <div class="S2Tag-content">${data.name}</div>
      <div class="S2Tag-extra">${data.id}</div>
    </div>`;
}
 ...
 */
function tokenizer(input, selection, selectCallback) {

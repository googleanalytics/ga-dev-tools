    ...
    for (let i = 0; i < results.length; i++) {
      searchResults.append(
        '<tr class="view-result"><td>' +
          ...
          "</svg></a> " +
          '<div class="AccountExplorerResults-id">' +
          highlightSafe(results[i].view.id) +
          "</div></td><td><div " +
          'class="AccountExplorerResults-id">' +
          highlightSafe("ga:" + results[i].view.id) +
          "</div></td></tr>"
      );
    }
  }
...
function getAllIds(summaries, ids, returnObject) {
  ...
  if (ids.viewId) {
    ...
    accountObject.account = summaries.getAccountByWebPropertyId(ids.propertyId);
    // pick first profile
    ...
    accountObject.view = summaries.getProperty(
      accountObject.property.id
    ).views[0];
  } else {

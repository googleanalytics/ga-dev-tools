...
2. Create a template file at `templates/<project-slug>/index.html`, where `<project-slug>` is the value from `meta.yaml`. If the project has pages, create additional files at `templates/<project-slug>/<page-slug>.html`. Refer to the `templates/embed-api` directory for an example of a project the contains pages.

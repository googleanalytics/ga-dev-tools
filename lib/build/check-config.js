...
import { RuntimeJsonPath, Encoding, DotEnvProductionPath, AnswerNames, DotEnvDevelopmentPath, } from "./types.js";
export const writeEnvFile = async (config, endpointName = "bitly_auth") => {
    ...
    [DotEnvProductionPath, DotEnvDevelopmentPath].map(path => fs.writeFileSync(path, [gapiLine, bitlyLine, authEndpointLine, measurementIdLine].join("\n"), {
        encoding: Encoding,
    }));
};
...
const configQuestions = (filter) => {
    ...
    return [
        ...
        },
        {
            ...
            when: () => {
                return (filter.askAll || filter?.production?.firebaseProjectId === undefined);
            },
        },
        {
        ...
        },
        {
            ...
            when: () => {
                return (filter.askAll || filter?.production?.bitlyClientSecret === undefined);
            },
        },
        {
...
export const checkConfig = async (args) => {
    ...
    if (exists) {
        ...
        // values.
        currentConfig = JSON.parse(fs.readFileSync(RuntimeJsonPath, { encoding: Encoding }));
    }
...
const toRuntimeJson = (answers, currentConfig) => {
    const production = {
        measurementId: throwIfUndefined(answers.measurementIdProd || currentConfig?.production.measurementId),
        baseUri: answers.baseUriProd ||
            currentConfig?.production?.baseUri ||
            "http://localhost:5000",
        firebaseProjectId: throwIfUndefined(answers.firebaseProjectIdProd ||
            currentConfig?.production?.firebaseProjectId),
        gapiClientId: throwIfUndefined(answers.gapiClientIdProd || currentConfig?.production?.gapiClientId),
        bitlyClientId: throwIfUndefined(answers.bitlyClientIdProd || currentConfig?.production?.bitlyClientId),
        bitlyClientSecret: throwIfUndefined(answers.bitlyClientSecretProd ||
            currentConfig?.production?.bitlyClientSecret),
    };
    const staging = {
        measurementId: throwIfUndefined(answers.measurementIdStaging || currentConfig?.staging.measurementId),
        // TODO - This could be a bit smarter. Especially if we support changing the port.
        baseUri: answers.baseUriStaging ||
            currentConfig?.staging?.baseUri ||
            "http://localhost:5000",
        firebaseProjectId: throwIfUndefined(answers.firebaseProjectIdStaging ||
            currentConfig?.staging?.firebaseProjectId),
        gapiClientId: throwIfUndefined(answers.gapiClientIdStaging || currentConfig?.staging?.gapiClientId),
        bitlyClientId: throwIfUndefined(answers.bitlyClientIdStaging || currentConfig?.staging?.bitlyClientId),
        bitlyClientSecret: throwIfUndefined(answers.bitlyClientSecretStaging ||
